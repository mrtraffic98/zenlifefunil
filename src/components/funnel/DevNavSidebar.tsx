      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{currentStep} / {totalSteps}</p>
          </div>
          <button
            onClick={() => setIsOpen(false)}
        ...
        <div className="p-3 bg-primary/10 border-b border-border">
          <p className="text-xs text-muted-foreground mb-1">Paso actual:</p>
          <p className="font-semibold text-primary text-sm">{stepNames[currentStep - 1]?.replace(/^\d+\.\s*/, '')}</p>
        </div>
        ...
        <div className="overflow-y-auto h-[calc(100%-120px)]">
          {stepNames.map((name, index) => {
            ...
            const isSectionStart = stepNum === 1 || stepNum === 4 || stepNum === 9 || stepNum === 12 || 
                                  stepNum === 15 || stepNum === 22 || stepNum === 27 || stepNum === 31 || 
                                  stepNum === 33 || stepNum === 35 || stepNum === 37;
            
            return (
              <div key={stepNum}>
                {isSectionStart && stepNum > 1 && (
                ...
                >
                  <span className={`text-xs font-mono w-8 flex-shrink-0 ${isActive ? "text-primary" : "text-muted-foreground"}`}>
                    {stepNum.toString().padStart(2, '0')}
                  </span>
                  <span className="truncate flex-1">{name.replace(/^\d+\.\s*/, '')}</span>
                  {isActive && <ChevronRight className="w-3 h-3 flex-shrink-0 text-primary" />}
                </button>
              </div>
            );
          })}
